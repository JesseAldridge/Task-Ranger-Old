...
BaseTree.prototype.after_bind_intervals = function() {
  ...
  this.scope.delete_node = function(node) {
    var scope = tree.scope,
        deleted_nodes = [],
        deleted_root = scope.nodes[node.node_id]
  ...
  this.scope.delete_interval = function(node, interval) {
    var node_id = node.node_id
        node_intervals = tree.scope.nodes[node_id].node_intervals,
